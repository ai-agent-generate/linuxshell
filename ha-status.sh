...
load_linuxshell_modules() {
  ...
  if [[ -n "${LINUXSHELL_MODULE_ROOT}" ]]; then
    # 二次及以后调用：复用首次确定的 root（本地仓库根或远程 tmpdir），避免重复 mktemp/下载与 BASH_SOURCE 漂移
    module_root="${LINUXSHELL_MODULE_ROOT}"
  else
    script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" 2>/dev/null && pwd -P || pwd)"
    if [[ -f "${script_dir}/lib/status-common.sh" ]]; then
  ...
  if [[ "${LINUXSHELL_MODULE_SOURCE}" == "remote" ]]; then
    for module in "$@"; do
      mkdir -p "${module_root}/$(dirname "$module")"
      [[ -f "${module_root}/${module}" ]] && continue
